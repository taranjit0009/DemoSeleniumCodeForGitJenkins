...
@pytest.mark.usefixtures("forData")
class TestHome(BaseClass):
    ...
    @pytest.mark.testby
    def test_static_dropdown (self,forData):
        log= self.getLogger()
        try:
            TestHome.driver.find_element(By.XPATH,"//body/app-root[1]/form-comp[1]/div[1]/form[1]/div[1]/input[1]").send_keys(forData["Name"])
            TestHome.driver.find_element(By.XPATH,"//body/app-root[1]/form-comp[1]/div[1]/form[1]/div[2]/input[1]").send_keys(forData["Email"])
            TestHome.driver.find_element(By.ID,'exampleInputPassword1').send_keys(forData["password"])
